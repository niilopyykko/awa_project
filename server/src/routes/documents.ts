...
router.delete(
    ...
    async (req: CustomRequest, res: Response) => {
        try {
            ...
            if (String(doc.owner) !== String(userId)) return res.status(403).json({ message: 'Only owner can permanently delete' });
...
router.get(
    ...
    router.post(
        ...
        async (req: CustomRequest, res: Response) => {
            try {
                ...
                if (doc.owner.toString() !== req.user!.id) return res.status(403).json({ message: 'Only owner can manage sharing' });

                const result: any = {};

                if (collaborator) {
                    // find user by username
                    const user = await User.findOne({ $or: [{ username: collaborator }, { email: collaborator }] }).select('_id');
                    if (!user) return res.status(404).json({ message: 'Collaborator not found' });
                    const uid = user._id;
                    if (!doc.editors) doc.editors = [] as any;
                    if (!doc.editors.map((e: any) => e.toString()).includes(uid.toString())) {
    ...
    router.get('/documents/:id', async (req: Request, res: Response) => {
        try {
            ...
            const isOwner = userId && doc.owner && String((doc.owner as any)._id || (doc.owner as any)) === String(userId);
